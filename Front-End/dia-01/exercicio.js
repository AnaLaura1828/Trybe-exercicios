// 1 - Dado o código abaixo, qual a ordem de finalização de execução das linhas comentadas? A ordem de execução é A,B e C
...
// 2 - Agora, dado o código abaixo, qual a ordem de finalização de execução das linhas comentadas? A ordem de finalização é Mars, Jupiter e venus
const planetDistanceFromSun = ({ name, distanceFromSun: { value, measurementUnit } }) =>
  `${name} is ${value} ${measurementUnit} apart from the Sun`;
...
//     const funcTemp = getMarsTemperature();
//     // setTimeout(() => console.log(`Mars temperature is: ${funcTemp} degree Celsius `), messageDelay());
// }
...
//              setTimeout(() => callback(funcTemp), messageDelay());
//              setTimeout(() => err(errors), messageDelay());
//     }
...
const pokemons = [
  ...
  getPokemonDetails((pokemon) => pokemon.name === 'Charmander',(elem, param) => {
      if (elem) {
        console.log(elem);
      } else {
        console.log(param);
